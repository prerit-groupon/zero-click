...
dotenv.config({ path: resolve(__dirname, '../gcp-billing/.env') });

const KEY_FILE   = resolve(__dirname, '../gcp-billing/service-account-key.json');
const PROJECT    = process.env.GCP_BILLING_PROJECT;
const DATASET    = process.env.GCP_BILLING_DATASET;

async function run() {
  const bq = new BigQuery({ projectId: PROJECT, keyFilename: KEY_FILE });
